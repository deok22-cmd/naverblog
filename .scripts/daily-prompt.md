...
**큰 틀**: 네이버 원고 5건 → 같은 주제로 티스토리 미러 5건(제목·본문 새로 작성, 이미지는 네이버 것을 그대로 재사용) → 인스타 카드 5건. 이게 전부.
...
성공:
```
[DAILY OK YYYY-MM-DD] 5 naver + 5 tistory created in output/YYMMDD/ and output_tistory/YYMMDD/, trackers updated, 0 git commits
```
