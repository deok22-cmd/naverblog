...
- **Phase A (네이버)**: 5건의 네이버 블로그 원고를 `output/YYMMDD/`에 작성, 트래커 갱신.
  - 발행 구성: **트랙 A 2건 + 트랙 B 2건 + 트랙 C 1건** (`Naverblog.md` 02조)
...
2. `국내여행지.md` — 국내여행 주제 데이터베이스 (트랙 prefix `[A]/[B]/[C]` 체계)
...
### 2.2 신규 주제 선정 (Fresh 모드 전용) — v2 트랙 룰 (2+2+1)
`Naverblog.md` 02조의 **2+2+1 트랙 룰**을 따른다. 서브 카테고리(스프레드시트/레시피)는 발행하지 않는다.

`국내여행지.md`의 `02 | 향후 작성 예정 테마` 섹션에서 상태값이 `작성대기중`인 행 중, **트랙 prefix(`[A]/[B]/[C]`)별로 다음 건수를 가장 위(낮은 일련번호)에서부터 순차 선택**한다.

| 트랙 | 선정 건수 | 선정 기준 |
|---|---|---|
| **`[A]` 수도권 마이크로** | 2건 | `[A]` prefix 행 중 작성대기중 최상단 2건 |
| **`[B]` 시즌 D-7 미리보기** | 2건 | `[B]` prefix 행 중 작성대기중 최상단 2건. **단, "주요 기간" 컬럼의 행사 시작일이 오늘 기준 D-7~D-35 범위 외(특히 D-1 이내 또는 D-36 이상)인 행은 건너뛰고 다음 후보로 이동** |
| **`[C]` 지방 니치** | 1건 | `[C]` prefix 행 중 작성대기중 최상단 1건 |

**큐 부족 시 처리**:
- 어느 한 트랙에서 작성대기중 행을 채우지 못할 경우(예: `[B]` 큐가 0건), 본 실행을 **중단하지 말고** 부족분을 다른 트랙으로 메꿔 5건을 완성한다 (우선순위: A → C → B 추가 1건).
- 큐 잔여가 트랙별 임계치 이하(A 5건, B 7건, C 3건)면 본 실행 완료 후 `trackabc.md` §6의 발굴 트리거(T1)를 사용자에게 알린다.

**금지**:
- 본 실행에서 **`국내여행지.md`에 신규 주제를 자동 추가하지 않는다**. 주제 풀 보충은 `Naverblog.md` 02-bis조의 표준 절차(사용자 컨펌)로만 수행한다.
- 트랙 prefix가 없는 legacy 행(58~187 등 v1 발행 완료분)은 신규 선정 대상이 아니다.
...
| No | 트랙 | 주제 (트랙 prefix 포함) | 상태 |
|---|---|---|---|
| 1 | A | `[A] (선정된 수도권 마이크로 주제 1)` | ⏳ 작성 대기 |
| 2 | A | `[A] (선정된 수도권 마이크로 주제 2)` | ⏳ 작성 대기 |
| 3 | B | `[B] (선정된 시즌 D-7 주제 1)` | ⏳ 작성 대기 |
| 4 | B | `[B] (선정된 시즌 D-7 주제 2)` | ⏳ 작성 대기 |
| 5 | C | `[C] (선정된 지방 니치 주제 1)` | ⏳ 작성 대기 |

### Resume 모드의 경우
대시보드는 이미 존재하므로 신규 생성하지 않는다. 다만 기존 `✔️ 작성 완료` 상태인 행에 대해 다음을 수행한다.

- 해당 행의 주제명·슬러그를 식별하고, 그 슬러그가 `국내여행지.md`의 어느 항목에 매칭되는지 추적
- 추적이 되면 본 단계 5.2의 트래커 갱신 작업 시 해당 항목을 `작성완료(MM.DD)`로 함께 마킹 (트랙 prefix는 유지)
...
- **제목 작성 — `Naverblog.md` 03조 3.1 준수 (2026-05-18 발행분부터)**: 제목에 `2026`을 기계적으로 넣지 않는다(5건 중 최대 1~2건, 연도가 검색어 핵심일 때만). `완전 가이드`·`완벽 가이드`·`총정리`·`끝판왕` 등 클리셰 수식어 금지. 병렬 키워드 묶음은 쉼표(,) 대신 가운뎃점(·) 사용하되 제목 전체는 자연스러운 한 문장 유지.
...
- 본 실행에서 작성한 주제의 상태값만 `작성대기중` → `작성완료(MM.DD)`로 변경
- 트랙 prefix `[A]/[B]/[C]`는 그대로 유지
- 본 실행에서 작성하지 않은 다른 행은 절대 건드리지 않는다

**큐 잔여 알림** (조건: 작성 완료 후 트랙별 작성대기중 카운트가 임계치 이하일 때):
- 임계치: A 5건, B 7건, C 3건 (`trackabc.md` §6.3 기준)
- 임계치 미달 시 완료 출력에 `[QUEUE LOW: A=N, B=N, C=N]` 메시지 추가 — 사용자가 다음날 발굴 작업(02-bis조 T1)을 트리거하도록 안내
