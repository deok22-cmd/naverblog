...
> ## 🚫 절대 금지 시그니처 — 출력에 다음 문자열이 *한 개라도* 들어가면 게이트 FAIL (2026-05-20 강화, 실제 사고 대응)
>
> 네이버 원고의 **이미지 placeholder 박스**(`<div class="img-placeholder">` 구조)를 티스토리 미러로 그대로 옮기는 사고가 반복됐다(2026-05-19 #4·#5, 2026-05-20 5건 전부). 티스토리 미러는 이 박스 대신 **표준 `<img>` 블록만** 사용해야 한다. 출력 직전 본문에서 다음 토큰을 grep 해 *0건*임을 반드시 확인한다:
> - `이미지 프롬프트 (placeholder)` / `📷 AI 이미지 생성 영역` / `프롬프트 복사하기`
> - `class="prompt-text"` / `class="img-placeholder"` / `id="prompt-`
> - `<style` 블록 / ` class="` 속성 (어떤 셀렉터든)
> - `>작성일 20` 부제 줄 / 비팔레트 색 `#4a8c4a` `#2e7da6` `#1a4f6e` `#2d5a2d` `#3d7a3d` `#246090` `#e6c840`
>
> 이미지는 무조건 다음 한 줄짜리 표준 블록만 사용:
> ```html
> <div style="margin:30px 0;text-align:center;"><img alt="…" src="../../images/YYMMDD/<seq>_<slug>_<idx>.png" style="max-width:100%;height:auto;border-radius:8px;"/><div style="font-size:0.9em;color:#666;margin-top:10px;font-style:italic;">… / AI 제작 이미지</div></div>
> ```

> ## ✅ 필수 포함 시그니처 — 모든 티스토리 미러 5건 *각각*에 반드시 존재
>
> 1. 외곽 래퍼 div: `<div style="font-family: 'Apple SD Gothic Neo', sans-serif; font-size: 16px; line-height: 1.9; color: #222; max-width: 780px; margin: 0 auto; padding: 16px;">` (정확히 이 시그니처)
> 2. h1 Platinum 시그니처: `font-weight: 800; border-bottom: 5px solid` 포함 (카테고리 색 `#00796b` 등)
> 3. h2 번호 prefix: `01.` `02.` … (9개 이상)
> 4. 추천 박스: `<strong>🔗 같이 보면 좋은 글</strong>` (또는 `같이 볼만한 글`) + 어제 폴더 링크 2개
> 5. **태그 칩 10개**: `<span style="display:inline-block; background:#f0f0f0; padding:3px 10px; ...">#키워드</span>` × 10 (네이버 원본 `.tag` 그대로 또는 동등) — *5건 전부 누락 사고(2026-05-20) 재발 금지*

> ## 🔎 출력 직전 자가검증 (모델이 저장 *전*에 직접 수행)
>
> 5건 각각에 대해 다음을 모두 확인하고, 하나라도 어긋나면 *그 파일을 다시 작성한 뒤* 저장한다(작업트리 청결 책임은 모델에게 있다):
> - [ ] 외곽 래퍼 1개 / Platinum h1 1개 / 번호 h2 9개+ / `<style>·class=` 0개
> - [ ] 위 "절대 금지 시그니처" 토큰 합계 = 0
> - [ ] 표준 `<img>` 블록만 사용 (placeholder 박스 0)
> - [ ] 추천 박스 존재 / 태그 칩 ≥ 10개
> - [ ] 비팔레트 색 0
>
> 이 단계를 통과해야 wrapper 게이트(`tistory-gate.ps1` F1~F8)도 자동 통과한다. 게이트 차단 시 push가 멈춰 사람이 사후 복구해야 하므로 *모델 단계에서 확실히 차단*하는 것이 비용이 가장 낮다.
